...
RUN <<EOT
  if [ $DISTRIB_MAJOR -eq 8 ]; then
    # OPSEXP-3749 workaround for bugged openssl3-devel package
    # dnf install -y dnf-plugins-core
    # dnf config-manager -y --set-enabled powertools
    # dnf install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm
    # dnf install -y openssl3-devel
    ARCH=$(uname -m)
    dnf install -y https://archives.fedoraproject.org/pub/archive/epel/8.9/Everything/$ARCH/Packages/o/openssl3-devel-3.2.1-1.2.el8.$ARCH.rpm https://archives.fedoraproject.org/pub/archive/epel/8.9/Everything/$ARCH/Packages/o/openssl3-libs-3.2.1-1.2.el8.$ARCH.rpm
